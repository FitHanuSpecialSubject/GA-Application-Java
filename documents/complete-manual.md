...
|Property 3|23|18|18:100|
### Common Annotation
- ( R\_i ) represents the Requirement for Property ( i )
- ( W\_i ) represents the Weight for Property ( i )
- ( P\_i ) represents the Value for Property ( i )

Example: ( R\_1 ) means Requirement of Property 1
...
The values of ( R\_2 ) and ( W\_1 ) are taken from Individual A, and ( P\_1 ) is taken from Individual B.

- ( W\_1 ) and ( P\_1 ) are straightforward, while ( R\_2 ) has more nuance.
- The syntax rules give different results for Requirement calculations:
1. **( x-- )** returns ( x )
1. **( x:y )** returns ( \frac{x + y}{2} )
1. **( x++ )** returns ( x )
1. **( x )** returns ( x )
...
[
P \times A(B) = \sum\_{i=1}^{N} R\_i(P\_i) \times W\_i
]

Where:

- ( N ) is the number of properties of an individual
- ( R\_i ) is the requirement function for property ( i ) of A
- ( W\_i ) is the weight for property ( i ) of A
- ( P\_i ) is the value for property ( i ) of B
-----
...
3. **( R\_i(P\_i) = x:y(P\_i) ):**

[
\begin{cases}
\frac{\frac{|y - x|}{2} - |\frac{x + y}{2}| + P\_i}{\frac{|y - x|}{2}} + 1 & \text{if } P\_i \in [x, y] \
0 & \text{else}
\end{cases}
]
...
[
\begin{cases}
0 & \text{if } P\_i < 0 \text{ or } P\_i > 10 \
0 & \text{if } |P\_i - x| > 7 \
1 & \text{if } |P\_i - x| > 5 \
\frac{10 - |P\_i - x|}{11} & \text{else}
\end{cases}
]
...
[
P \times A(B) = R\_1(P\_1) \times W\_1 + R\_2(P\_2) \times W\_2 + R\_3(P\_3) \times W\_3
]

Substituting the values from the tables:

[
= 12--(12) \times 12 + 13(13) \times 13 + 18:100(18) \times 23
]

This simplifies to:

[
= \frac{12 + |12 - 12|}{12} \times 12 + 0 \times 13 + \left( \frac{\frac{|100-18|}{2} - \frac{100 + 18}{2} + 18}{\frac{|100 - 18|}{2}} + 1 \right) \times 23
]
...
Details about how payoffs are calculated are explained [here](#payoff).

*Conflict* is an additional functionality that discourages any two players from choosing specific strategies at any iteration.
...
2\. Build the project
     ...
     maven setup instruction: How to install maven Win, Mac, Linux
     mvn clean install
